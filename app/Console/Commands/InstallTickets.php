...
use mpba\Tickets\Models\Status;
...
class InstallTickets extends Command
{
     ...
     * @var string
     */
    protected $signature = 'tickets:install';
     ...
     * @var string
     */
    protected $description = 'Sets up the database as required for running Tickets. Run this only once, before making any modifications to project files.';
    ...
    {
        $this->line("Welcome to Laravel Tickets!\n");
    ...
    private function enableTickets()
    {
        $_SERVER['ARTISAN_TICKETS_INSTALLING'] = true;

        file_put_contents(config_path('app.php'), str_replace(
            '//mpba\Tickets\TicketsServiceProvider::class,',
            'mpba\Tickets\TicketsServiceProvider::class,',
            file_get_contents(config_path('app.php'))
        ));

        app()->register(\mpba\Tickets\TicketsServiceProvider::class);
    }

    private function setupTicketit()
    {
        $this->line('Setting up Tickets...');

        $installController = new \mpba\Tickets\Controllers\InstallController;
    ...
    {
        $this->info("\nTickets installation completed!");
        $this->comment('Mailing is disabled by default.');
        $this->comment("Don't forget to configure mail in your .env file.");
        [$major, $minor] = explode('.', LaravelVersion::getLaravelVersion());
        $this->line("https://laravel.com/docs/$major.$minor/mail");
    }
    ...
    private function removeInstaller()
    {
        if ($this->confirm('Do you want to remove the installer script?')) {
            file_put_contents(app_path('Console/Kernel.php'), str_replace(
                'Commands\InstallTickets::class,',
                '',
                file_get_contents(app_path('Console/Kernel.php'))
            ));

            unlink(app_path('Console/Commands/InstallTickets.php'));
        }
    }
}
