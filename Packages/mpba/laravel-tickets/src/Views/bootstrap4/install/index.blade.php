...
<div class="container">
    <h1 style="text-align: center">{{ trans('ticket::install.initial-setup') }}</h1>
  <form class="form-signin" action="{{url('/tickets-install') }}" method="post" style="max-width: 500px">
          <input type="hidden" name="_token" value="{{ csrf_token() }}" />
        <h3 class="form-signin-heading">{{ trans('ticket::install.master-template-file') }}</h3>
        ...
        <div class="form-group" id="other-path-group" style="display: none">
            <label for="other_path">{{ trans('ticket::install.master-template-other-path') }}</label>
            <input type="text" id="other_path" name="other_path" class="form-control" />
            <span id="helpBlock" class="help-block">
                {{ trans('ticket::install.master-template-other-path-ex') }}
            </span>
        </div>

        <h3 class="form-signin-heading">{{ trans('ticket::install.admin-select') }}</h3>
        ...
        <span id="helpBlock" class="help-block">
            {{ trans('ticket::install.admin-select-help-block') }}
        </span>
        <br>

        <div class="well small" style="border: 1px solid #ccc">
            @if(!empty($inactive_migrations))
                <b>{{ trans('ticket::install.migrations-to-be-installed') }}</b>
                <ul>
            ...
            @else
                <b>{{ trans('ticket::install.all-tables-migrated') }}</b>
            @endif
        </div>
        <br>
        <button class="btn btn-lg btn-primary btn-block" type="submit">
            {{ trans('ticket::install.proceed') }}
        </button>
    </form>
